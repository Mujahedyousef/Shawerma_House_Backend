...
export class ProductRepository extends IProductRepository {
  ...
  async create(data) {
    const { technicalSpecs = [], colorIds = [], imageUrls = [], ...productData } = data;
    ...
    return await prisma.product.create({
      data: {
        ...
        technicalDataSheetUrl: productData.technicalDataSheetUrl || null,
        catalogUrl: productData.catalogUrl || null,
  ...
  async update(id, data) {
    const { technicalSpecs, colorIds, imageUrls, ...productUpdateData } = data;
    const updateData = {};
    ...
    if (productUpdateData.technicalDataSheetUrl !== undefined) updateData.technicalDataSheetUrl = productUpdateData.technicalDataSheetUrl || null;
    if (productUpdateData.catalogUrl !== undefined) updateData.catalogUrl = productUpdateData.catalogUrl || null;
      if (productUpdateData.similarProductIds !== undefined) updateData.similarProductIds = productUpdateData.similarProductIds || [];
      if (productUpdateData.categoryId !== undefined) updateData.categoryId = productUpdateData.categoryId || null;
    ...
    // Handle images update - replace all images with the provided list
    if (imageUrls !== undefined) {
      ...
      // Find images to add (in new list but not in DB)
      const imagesToAdd = imageUrls.filter(url => !existingImageUrls.includes(url));
      ...
      if (imagesToAdd.length > 0) {
        ...
        updateData.images = {
          create: imagesToAdd.map((imageUrl, index) => ({
            imageUrl: imageUrl,
      ...
      // Update order for remaining images to maintain sequence
      if (imageUrls.length > 0) {
        const remainingImages = existingImages.filter(img => imageUrls.includes(img.imageUrl));
        // Reorder remaining images based on new list order
        for (let i = 0; i < imageUrls.length; i++) {
          const imageUrl = imageUrls[i];
          const existingImage = remainingImages.find(img => img.imageUrl === imageUrl);
          if (existingImage && existingImage.order !== i) {
  ...
  async getFilterOptions() {
    const [categories, productTypes, brands, colors, countries, years] = await Promise.all([
    ...
    const [categoryCounts, productTypeCounts, brandCounts, colorCounts, countryCounts, yearCounts] = await Promise.all([
    ...
    // Create count maps
    const categoryCountMap = Object.fromEntries(categoryCounts.map(c => [c.categoryId, c._count]));
    ...
    const categoriesWithCount = categories.map(cat => ({ ...cat, productCount: categoryCountMap[cat.id] || 0 }));
